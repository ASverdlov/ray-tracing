...
class Transform {
 ...
 public:
  ...
  explicit Transform(const double*);
...
Transform Transform::operator*(double k) const {
  return Transform(
    matrix[0][0] * k, matrix[0][1] * k, matrix[0][2] * k,
    matrix[1][0] * k, matrix[1][1] * k, matrix[1][2] * k,
    matrix[2][0] * k, matrix[2][1] * k, matrix[2][2] * k);
}
